...
这是一个人类主导、AI 助手辅助的量化投资系统。
主要使用coding plan(百度&&讯飞，以及openai go)，工具为claude code，模型主要包括glm-5.1、deepseek-v4-flash、glm5。
这是一个美好的时代，AI极大的放大了人类专家的能力边界、产出效率；难以想象在古法时代，实现这个规模的系统和服务，我们所要投入的开发、测试，所消耗的时间。
当然，现在还需要人类深入参与各个部分（主要是输出经验，明确的告诉AI应该如何设计、如何实现），AI 虽然知识很丰富，但放任不管，可能产出一个很糟糕的东西，系统的上限依旧是人来决定的：就像lead 一个团队一样。
docs 目录下集中了一些系统的设计文档，并有多轮人 -> AI 交互讨论记录，指导/约束 AI 架构与编码。

基于 Python DSL → IR 编译 → C++ 执行的量化投资系统，支持多策略回测与实盘交易（待完善）。
麻雀虽小，五脏俱全，这是一个主打高性能的图计算系统，支持策略以python 开发、提交与高效执行；同时有完整的数据存储通路；
这些基础设施并不只限于量化系统，也可以推广应用于搜索、推荐等系统。
系统还处在高速迭代阶段，正在快速完善功能与性能。
## 架构概览
...
### 线程亲和协程模型
现代CPU 架构下，线程模型对性能的影响是显著的。这一个版本的演进，依旧用到了一些OS 系统调用（如IO 栈 使用io_uring), 
以及，一些三方SDK的调用可能存在阻塞，为充分并行利用多核，使用的work-stealing 调度机制。
后续，会考虑全用户态化（spdk 本地盘IO 、 DPDK/RDMA 网络IO), 构建出完整的RTC(run to complete)在线任务模型。
现在，内部还有一些阻塞性原语存在，也正在清理替代中。
